...
@section('isihalaman')
    <h3><center>Daftar Buku Perpustakaan Meraih Mimpi</center></h3>
    ...
    <table class="table table-bordered table-striped">
        ...
        @foreach ($buku as $index=>$bk)
                <tr>
                    <td align="center" scope="row">{{ ($buku->currentpage()-1) * $buku->perpage() + $loop->index + 1 }}</td>
                    <td>{{$bk->id}}</td>
                    <td>{{$bk->kode_buku}}</td>
                    <td>{{$bk->judul}}</td>
                    <td>{{$bk->pengarang}}</td>
                    ...
                    <td align="center">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form id="formbukuedit{{ $bk->id }}" action="{{ url('bukuedit/edit/'.$bk->id) }} " method="post" enctype="multipart/form-data">
                                            @csrf
                                            {{ method_field('PUT') }}
                        ...
                        |
                        <a href="{{ url('buku/hapus/'.$bk->id) }}" onclick="return confirm('Yakin mau dihapus?')">
    ...
    <div class="modal fade" id="modalBukuTambah" tabindex="-1" role="dialog" aria-labelledby="modalBukuTambahLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formbukutambah" action="{{ url('bukutambah') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            <label for="id_buku" class="col-sm-4 col-form-label">Kode Buku</label>
                            <div class="col-sm-8">
